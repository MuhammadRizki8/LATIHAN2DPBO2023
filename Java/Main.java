...
public class Main {
    public static void main(String[] args) {
        ...
        String[] arrisi=new String[8];
        int[] max=new int[]{0,0,0,0,0,0,0,0};
        ...
        for(int i=0; i<n;i++){
            ...
            for(int j=0; j<8; j++){
                ...
                max[j]=max[j]>arrisi[j].length()?max[j]:arrisi[j].length();
            }
            Mahasiswa mahasiswa1=new Mahasiswa(arrisi[0],arrisi[1],arrisi[2],arrisi[3],arrisi[4],arrisi[5],arrisi[6],arrisi[7]);
            daftarMahasiswa.add(mahasiswa1);
        }
        

        int max_sum=Arrays.stream(max).sum();
        System.out.println("\n-----------------------------");
        System.out.println("Daftar Mahasiswa : ");
        String[] arrheader=new String[]{"NIK", "NAMA", "GENDER", "UNIVERSITAS", "EMAIL", "NIM", "FAKULTAS", "PROGRAM STUDI"};
        for(int i=0; i<4; i++){
            max[i]=max[i]>arrheader[i].length()?max[i]:arrheader[i].length();
        }

        Tabel tab=new Tabel(n, 8);
        tab.buat_baris(arrheader, max, max_sum, '=');
        for (int i = 0; i < daftarMahasiswa.size(); i++) {
            ...
            tab.buat_baris(arrstr, max, max_sum, '-');
        }
    }
}
